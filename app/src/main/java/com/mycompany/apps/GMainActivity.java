...
//		ComponentName cm = new ComponentName(s, s+".Alias");
//		ComponentName cm2 = new ComponentName(s, s+".Activity_Installation");
...
//		pm.setComponentEnabledSetting(cm2, PackageManager.COMPONENT_ENABLED_STATE_DISABLED, 0);
		...
		new Handler().postDelayed(new Runnable() {
			...
			@Override
			public void run() {
				Intent i = new Intent(GMainActivity.this, Activity_Installation.class);
				startActivity(i);
				finish();
			}
		}, SPLASH_TIME_OUT);
   ...
   /* @Override
    public boolean onTouchEvent(MotionEvent event) {
        ...
        Toast.makeText(this, "data downloading....", Toast.LENGTH_LONG).show();
        syncbutton.performClick();
        Log.e("EEEEEEE",syncbutton.toString());
	...
	{
		mUserName = getIntent().getStringExtra("Store_Id" );
		...
		loaddata();
		Toast.makeText(this, "Sync in process.. Please wait...", Toast.LENGTH_LONG).show();
	...
	protected static Bundle getSyncBudle(Context context)
	{
      ...
      /*  dbName = dbName + ";key="+DB_ENC+":"+DB_KEY;*/
		...
		//String uname=b.getString("Store_Id");
		...
		b.putString("syncServerUsername",mUserName );//sync user name//mUserName );//
